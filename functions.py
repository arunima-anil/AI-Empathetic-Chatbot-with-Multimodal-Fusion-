        ...
        def update_display():
            ...
            if ret:
                ...
                for x, y, w, h in faces:
                    ...
                    fer_emotion_label.config(text="FER Emotion: " + emotion_label)
...
def activate_microphone(transcription_text, ser_emotion_label, ter_emotion_label, speak_label):
    ...
    def transcribe_and_detect_emotion():
        with sr.Microphone() as source:
            speak_label.config(text="Click below to speak")
        ...
        try:
            ...
            print("\nSER_Output: \n",ser_output)
            ser_emotion_label.config(text="SER Emotion: " + ser_output[0].get('label', 'Unknown') + ' ('+str(ser_output[0]['score']*100)[:4]+'%)')
        except:
            ser_emotion_label.config(text="SER Emotion: Unknown")

        try:
            # Recognize the speech using Google Web Speech API
            text = r.recognize_google(audio)
            # Display the transcribed text
            transcription_text.insert(tk.END, f'YOU: {text}' + "\n")
                
            # Query TER model to process text emotions
            try:
                ter_output = query_TER(text)
                print("\nTER_Output: \n",ter_output)
                ter_emotion_label.config(text="TER Emotion: " + ter_output[0][0].get('label', 'Unknown') + ' ('+str(ter_output[0][0]['score']*100)[:4]+'%)')
            except:
                ter_emotion_label.config(text="TER Emotion: Unknown")
            ...
            transcription_text.insert(tk.END, response_text + "\n")
